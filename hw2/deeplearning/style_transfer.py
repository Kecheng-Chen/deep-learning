...
def content_loss(content_weight, content_current, content_target):
    ...
    #                               YOUR CODE HERE                               #
    ##############################################################################
    i, C_l, H_l, W_l = content_current.size()
    F_l = content_current.view(C_l, H_l*W_l)
    P_l = content_target.view(C_l, H_l*W_l)
...
def gram_matrix(features, normalize=True):
    ...
    gram = torch.mm(features.view(1, C, H*W)[0,:,:], features.view(1, C, H*W)[0,:,:].transpose(1,0))
    if normalize == True:
      gram /= (H*W*C)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for index in range(len(style_layers)):
      layer = style_layers[index]
      gram = gram_matrix(feats[layer])         
      style_loss += style_weights[index] * torch.sum((gram - style_targets[index])**2)       
    return style_loss
