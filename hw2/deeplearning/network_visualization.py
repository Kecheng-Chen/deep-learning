...
def compute_saliency_maps(X, y, model):
    ...
    saliency = torch.autograd.grad(outputs=scores, inputs=X, grad_outputs=torch.ones_like(y))
...
def make_fooling_image(X, target_y, model):
    ...
    for ii in range(100):
        ...
        saliency, i = scores.data.max(dim=1)
        if i[0] == target_y:
            break
        score = scores[0,target_y]
        grad = torch.autograd.grad(outputs=score, inputs=X_fooling)[0]
